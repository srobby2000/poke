...
import { AnimationMixer, Box3, DoubleSide, Group, Mesh, MeshStandardMaterial, Vector3 } from "three";
...
type Props = { fitPreview?: boolean; species: string; color?: string; hit?: boolean; walking?: boolean };
...
export function PokemonModel({ species, hit = false, walking = false, fitPreview = false }: Props) {
  ...
  return <ModelBoundary key={`${species}-image-v2`}><AsyncPokemon number={model.number} heightM={model.heightM} fitPreview={fitPreview} hit={hit} walking={walking} /></ModelBoundary>;
}

function AsyncPokemon({ number, heightM, fitPreview, hit, walking }: { number: number; heightM: number; fitPreview: boolean; hit: boolean; walking: boolean }) {
  ...
  return <LoadedPokemon number={number} heightM={heightM} fitPreview={fitPreview} gltf={result} hit={hit} walking={walking} />;
}

function LoadedPokemon({ number, heightM, fitPreview, gltf, hit, walking }: { number: number; heightM: number; fitPreview: boolean; gltf: GLTF; hit: boolean; walking: boolean }) {
  ...
  const model = useMemo(() => {
    ...
    const legs: { bone: Group; angle: number; side: number }[] = [];
    scene.traverse(object => {
      if (/^(LThigh|RThigh|LArm|RArm)$/.test(object.name)) {
        legs.push({ bone: object as Group, angle: object.rotation.x, side: object.name.startsWith("L") ? 1 : -1 });
      }
    });
    return { scene, scale, offset: [-center.x * scale, -bounds.min.y * scale, -center.z * scale] as [number, number, number], materials, legs };
  }, [source, number, heightM, fitPreview]);
  const animatedLegs = useRef(model.legs);
  useEffect(() => { animatedLegs.current = model.legs; }, [model]);
  const mixer = useMemo(() => new AnimationMixer(model.scene), [model.scene]);
  useEffect(() => {
    const idle = animations.find(clip => /defaultwait|idle/i.test(clip.name));
    if (idle) mixer.clipAction(idle).play();
    return () => { mixer.stopAllAction(); mixer.uncacheRoot(model.scene); };
  }, [animations, mixer, model.scene]);
  useEffect(() => {
  ...
  useEffect(() => () => { model.scene.traverse(object => { if (object instanceof Mesh) for (const material of Array.isArray(object.material) ? object.material : [object.material]) material.dispose(); if (number === 12 && object instanceof Mesh) object.geometry.dispose(); }); }, [model, number]);
  useFrame(({ clock }, delta) => {
    mixer.update(delta);
    for (const { bone, angle, side } of animatedLegs.current) bone.rotation.set(angle + (walking ? Math.sin(clock.elapsedTime * 10) * 0.35 * side : 0), bone.rotation.y, bone.rotation.z);
    if (root.current) root.current.rotation.z = walking ? Math.sin(clock.elapsedTime * 10) * 0.025 : 0;
  });
