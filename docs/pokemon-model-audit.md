...
### Softer size differences and cohesive village scenery

Following visual feedback, arena and companion display heights now use `1.1 * heightM ** 0.35` instead of literal metre scale. This retains species ordering while making the extremes less distracting; Pokédex measurements remain unchanged. Squirtle displays at about 0.86 scene units and Lapras at 1.52. Name labels use the same display-height conversion.

Rectangular building footprints with doors render as complete houses with pitched roofs, foundations, timber trim, framed windows, doors, steps, and chimneys. Other wall layouts retain their original tile footprints. Trees use instanced layered crowns and trunks, and pond tiles join into continuous water with banks only along exposed edges, shoreline stones, lily pads and ripple accents. Fences now have posts and rails. Daylight, terrain and material colors share a muted village palette. Collision maps and door interaction coordinates are unchanged. The combined house/tree/pond/trainer scene was exported and visually inspected offline: [village preview](model-review/village.png).
